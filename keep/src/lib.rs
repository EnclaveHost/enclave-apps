//! keep: a confidential notebook backed by a wallet-unlocked encrypted volume.
//!
//! Built on the encrypted-volumes model (rclone crypt over S3). A deployment's
//! App Config names WHERE each volume's ciphertext lives (`encVolumes`: S3
//! endpoint, bucket, path) - never a key. The platform preopens an empty dir
//! per volume at /enc/<name> (names in ENCLAVE_ENC) and starts the app
//! immediately. This app serves the notebook UI: the owner unlocks with their
//! wallet (or an rclone-crypt password) in the browser, over the deployment's
//! in-enclave-terminated TLS, and the app forwards it to the manager's loopback
//! /encvol plane (ENCLAVE_ENC_API, authenticated with ENCLAVE_ENC_TOKEN - which
//! never leaves the enclave and never reaches the browser). The manager pulls
//! the ciphertext from the bucket and decrypts it into the live preopen; the
//! notes then appear here as ordinary std::fs reads AND WRITES.
//!
//! keep extends the read-only sample into a read-WRITE notebook: Save writes
//! plaintext into the enclave's decrypted ramdisk with plain std::fs, Sync
//! re-encrypts and pushes ciphertext back to the bucket, Lock wipes the
//! plaintext. The bucket and the host only ever see ciphertext.
//!
//! Routes:
//!   GET  /               - notebook UI (self-contained HTML).
...
//!   POST /api/sync       - proxied POST <ENCLAVE_ENC_API>/sync       {name}  (re-encrypt local edits back to the bucket)
//!   POST /api/lock       - proxied POST <ENCLAVE_ENC_API>/lock       {name}  (wipe plaintext, drop credentials)
//!   POST /api/write      - {vol, path, content}  write a note into /enc/<vol>/<path> (std::fs, <= 1 MiB).
//!   POST /api/delete     - {vol, path}           remove a note from the volume.
//!   POST /api/mkdir      - {vol, path}           create a directory in the volume.
...
//! S3 client - once unlocked, /enc/<name> is a normal directory you can read
//! and write with std::fs. The bucket and the operator's host only ever saw
...
const MAX_CONTENT: usize = 1024 * 1024; // a saved note caps at 1 MiB of plaintext
const MAX_WRITE_BODY: usize = MAX_CONTENT + 100 * 1024; // ~1.1 MiB: note + JSON framing
...
/// Drain the browser's request body with a caller-chosen cap - the note-write
/// route carries up to ~1 MiB of note content, far past MAX_BODY. An oversize
/// body is truncated here (its JSON then fails to parse, yielding a 400).
fn read_request_body_capped(req: &IncomingRequest, cap: usize) -> Vec<u8> {
    let mut out = Vec::new();
    let Ok(body) = req.consume() else { return out };
    let Ok(stream) = body.stream() else { return out };
    loop {
...
/// POST /api/write {vol, path, content}: write a note into the decrypted
/// preopen with plain std::fs (create_dir_all the parent first). The content
/// caps at MAX_CONTENT; the path is validated by `resolve` (no traversal, no
/// unknown volumes).
fn handle_write(out: ResponseOutparam, body: &[u8]) {
    let Ok(v) = serde_json::from_slice::<serde_json::Value>(body) else {
        return json_err(out, 400, "invalid JSON body");
    };
    let vol = v.get("vol").and_then(|x| x.as_str()).unwrap_or("");
    let path = v.get("path").and_then(|x| x.as_str()).unwrap_or("");
    let content = v.get("content").and_then(|x| x.as_str()).unwrap_or("");
    if content.len() > MAX_CONTENT {
        return json_err(out, 400, "note exceeds the 1 MiB limit");
    }
    let Some(disk) = resolve(vol, path) else {
        return json_err(out, 400, "unknown volume or bad path");
    };
    if let Some(parent) = disk.parent() {
        if std::fs::create_dir_all(parent).is_err() {
            return json_err(out, 500, "could not create the note's parent directory");
        }
    }
    match std::fs::write(&disk, content.as_bytes()) {
        Ok(()) => respond_bytes(
            out,
            200,
            "application/json",
            serde_json::json!({ "ok": true, "size": content.len() })
                .to_string()
                .as_bytes(),
        ),
        Err(e) => json_err(out, 500, &format!("write failed: {e}")),
    }
}
...
fn handle_delete(out: ResponseOutparam, body: &[u8]) {
    ...
    match std::fs::remove_file(&disk) {
        Ok(()) => respond_bytes(out, 200, "application/json", b"{\"ok\":true}"),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            json_err(out, 404, "no such file in this volume")
        }
        Err(e) => json_err(out, 500, &format!("delete failed: {e}")),
    }
}

/// POST /api/mkdir {vol, path}: create a directory in the volume.
fn handle_mkdir(out: ResponseOutparam, body: &[u8]) {
    let Ok(v) = serde_json::from_slice::<serde_json::Value>(body) else {
        return json_err(out, 400, "invalid JSON body");
    };
    let vol = v.get("vol").and_then(|x| x.as_str()).unwrap_or("");
    ...
    match std::fs::create_dir_all(&disk) {
        Ok(()) => respond_bytes(out, 200, "application/json", b"{\"ok\":true}"),
        Err(e) => json_err(out, 500, &format!("mkdir failed: {e}")),
    }
}
    ...
    fn handle(req: IncomingRequest, out: ResponseOutparam) {
        ...
        match (req.method(), path) {
            ...
            (Method::Post, "/api/write") => {
                let body = read_request_body_capped(&req, MAX_WRITE_BODY);
                handle_write(out, &body)
            }
            ...
            _ => json_err(
                out,
                404,
                "not found; routes: GET /, GET /api/status, POST /api/{unlock,sync,lock,write,delete,mkdir}, GET /ls, GET /f/<volume>/<path>, GET /ping",
            ),
        }
    }
}
