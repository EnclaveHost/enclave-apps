...
(on ramdisk) → *synced* (pushed to your bucket). You always know whether an
...
filter as `/f/` (no `..`, no absolute jumps, no volumes outside
`ENCLAVE_ENC`), and `write` caps a note at 1 MiB. The unlock/sync/lock routes are relayed
