...
//! signed-in account, and that name is who a per-user tool acts for.
//!
//! A port of eyesoff-ai's sso.rs (PLATFORM-sso.md there is the spec), kept
//! byte-compatible so one token means the same identity in every app that speaks it:
//!
...
//!
//! Like jot, this app accepts MORE THAN ONE audience.
//! Its own deployment id (the UI's sign-in) and the ids of the deployments
//! it serves tools to (an eyesoff-ai instance forwarding the token it
...
pub struct SsoConfig {
    ...
    /// instances this adapter serves, which forward the tokens they verified
...
pub fn verify(cfg: &SsoConfig, token: &str, now_secs: u64) -> Result<Claims, String> {
    ...
    if !hex_eq(&aud, &cfg.audience) && !cfg.accept.iter().any(|a| hex_eq(&aud, a)) {
        return Err("sign-in token was minted for a deployment this adapter does not serve".into());
    }
