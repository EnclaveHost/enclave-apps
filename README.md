...
| [jot](jot/) | A notebook your agent keeps in your own bucket: a keyed JSON API (list, read, write, append, search, delete) with every note one SigV4-signed object in R2/S3, written from inside the enclave. With an `sso` block it is one notebook per signed-in Enclave account, each sealed under its own derived key; `GET /api/tools` hands any agent its function schemas, and an eyesoff-ai `tools.http` block, ready to paste. |
