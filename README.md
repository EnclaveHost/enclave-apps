...
| [network-test](network-test/) | Answers "who am I on the network?", demonstrating dedicated-IP egress and declared ports. |
...
| [eyesoff-ai](eyesoff-ai/) | Private LLM chat on attested GPUs; prompts, replies, and history never leave the enclave and your browser. OpenAI-compatible API; ships no weights, serves whatever model volumes are attached. Pictures ride the same tool registry: it draws at the shape the request implies, reads what you attach, and upscales either. A page it writes renders in the chat itself, in a frame with no origin and no way out to the network, and downloads as a file. |
...
| [speech-reader](speech-reader/) | The other direction: speech to text with Granite Speech 4.1 2B (top of the Open ASR leaderboard, Apache-2.0), audio crossing to the host through the mtmd audio verb (PLATFORM.md is the spec), long recordings chunked at quiet points, OpenAI-compatible transcription API. What you record is transcribed to you and kept by no one. |
...
| [jot](jot/) | A notebook your agent keeps in your own bucket: a keyed JSON API (search, read, write, append, delete) with every note one SigV4-signed object in R2/S3, written from inside the enclave; search is hybrid BM25 plus vectors from an embedding model that runs inside the component, over an index that is one sealed object in the bucket. With an `sso` block it is one notebook per signed-in Enclave account, each sealed under its own derived key; `GET /api/tools` hands any agent its function schemas, and an eyesoff-ai `tools.http` block, ready to paste. |
| [api-mcp-adapter](api-mcp-adapter/) | Any HTTP API as an MCP server, from an attested enclave: describe endpoints in the app config (the same entries an eyesoff-ai `tools.http` block takes) and every MCP client gets one streamable-HTTP endpoint whose `tools/call` is a templated request the enclave makes with its own credentials. The API keys stay inside the guest, a caller chooses arguments and never a URL or a header, and pictures, citations and the signed-in user all cross intact. |
...
[Enclave Source-Available License v1.0](LICENSE): source published for
transparency, audit and verification; not open source. Vendored third-party
code (e.g. `risc-box/emu/`) remains under its own upstream license.
