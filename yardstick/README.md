...
*someone* who then **knows**: a spreadsheet owner, a survey vendor, that one
trusted colleague. yardstick replaces the someone with attested enclave
...
information exist in `/api/measure` only once the measurement closed
*at quorum*: `median`, `mean`, `p25`, `p75`, `fingerprints`. That asymmetry
is the app.
...
| `GET /api/measure?id=` | (none) | state; `+ revealed, median, mean, p25, p75, fingerprints` when closed at quorum |
| `POST /api/close` | `id=&admin=` | the closed state (idempotent) |
| `GET /api/mine?id=&token=` | (none) | your echo while open; `counted` after |
| `GET /api/stream?id=` | (none) | SSE `subs` / `closed` on topic `<id>` |
