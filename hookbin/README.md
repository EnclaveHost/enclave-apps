...
| `GET /api/bins/<token>/requests` | (none) | JSON array, oldest first |
| `POST /api/bins/<token>/response` | headers `x-status` (100–599), `x-ct`; body ≤ 8 KiB | `{ok}` |
| `POST /api/bins/<token>/clear` | (none) | `{ok}`; empties the ring, keeps the bin |
| `DELETE /api/bins/<token>` | (none) | `{ok}` |
...
`textContent` only, because captures are hostile input. SSE frames and list items
