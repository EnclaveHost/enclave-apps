...
impl VirtioBlockDisk {
	...
	pub fn load(&mut self, address: u64) -> u8 {
		...
		match address {
			...
			// Device config: capacity (u64 LE, in 512-byte sectors). risc-box
			// patch: report the ACTUAL loaded image size instead of a hardcoded
			// 100 MiB (0x032000 sectors). Upstream's constant silently caps
	...
	fn handle_disk_access(&mut self, memory: &mut MemoryWrapper) {
		...
		// risc-box patch: the driver can make SEVERAL buffers available under a
		// single queue-notify (the block layer batches requests, and more so as
		// the filesystem grows). Upstream processed exactly ONE descriptor chain
		// per notify, so on a larger rootfs the extra requests were never
		// completed and the guest hung forever in io_schedule waiting on them
		// (the ext2 sample happened to stay one-buffer-per-notify, so it booted).
		// Drain the whole avail ring: process every chain from our consumed
		// index up to the driver's published avail index.
		let avail_index = memory.read_halfword(base_avail_address.wrapping_add(2));
		while self.used_ring_index != avail_index {
