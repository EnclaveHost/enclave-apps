...
offer: **sealed** mode, where nobody can see the tally until the poll is
closed: not voters, not bystanders, not the platform operator, not *the
poll's own creator*. Any ordinary server can promise that; its operator can
also open a debugger. Here the counts exist only in the RAM of a
**hardware-attested
