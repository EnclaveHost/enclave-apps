...
| route | in | out |
|---|---|---|
| `GET /api/board` | (none) | `{w, h, palette, placed, version, painting, board}`; board is base64 of 16384 palette indices |
| `POST /api/px` | `i=<0..16383>&c=<0..15>&s=<session>` | `{ok, wait_ms}` · on cooldown `429 {wait_ms}` |
| `GET /api/stream?s=` | (none) | SSE: `px` = `{v, d:[[i,c],…]}` delta batches · `n` = `{painting, placed}`. `s=` is an opaque per-tab stream tag |
